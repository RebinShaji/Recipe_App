...
const Item= () => {
    ...
    const getData=async()=>{
        try{
          ...
          const res=await fetch(`https://api.edamam.com/api/recipes/v2?type=public&q=${searchString}&app_id=${APP_ID}&app_key=${APP_KEY}`, {
          method: 'GET',
          headers: {
    ...
    useEffect(()=>{
        ...
        return(
            <SafeAreaView style={styles.container}>
                ...
                <Text style={{ fontWeight: 'bold', color: "white", fontSize:40, alignItems:'center' }}>Recipes</Text>
                <TextInput
                type="text"
                name="searchBar"
                id="searchBar"
                ...
                /> 
                    ...
                    renderItem={({item}) => (
                        ...
                        <Text {...item.recipe.ingredientLines.map((label) => item.recipe.label) }
                         style={{ fontWeight: 'bold', color: 'white', fontSize:20, alignItems:'center' }}>{item.recipe.ingredientLines}</Text>
                    </View>
                    )}
                    />
                    </View>
